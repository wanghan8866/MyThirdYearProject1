...
from agents.snake_game_gen.neural_network import FeedForwardNetwork, linear, sigmoid, tanh, relu, leaky_relu, \
    ActivationFunction, \
...
class Snake(Individual, Env):
    ...
    def update(self, _action=-1):
        ...
        if self.is_alive:
            ...
            self.look()
            if _action not in self.possible_directions:
                # print("network", _action)
                # print("network", self.vision_as_array)
    ...
    def move(self) -> bool:
        ...
        if self._is_valid(next_pos):
            ...
            if next_pos == self.snake_array[-1]:
                ...
                if distance != 0:
                    self.steps_history.append(self._frames_since_last_apple / (distance + 1))
                self._frames_since_last_apple = 0
            # Normal movement
            else:
                # Move head
    ...
    def reset(self):
        ...
        if self.pattern is None:
            ...
            self.apple_location = self.pattern.apple
            # snake.reset()
            # snake = None
            self.snake_array = deque(self.pattern.snake_body)
    ...
    @staticmethod
    def create_snake_from_pattern(board_size: Tuple[int, int], pattern: Pattern, **kwargs):
        snake = Snake(board_size=board_size, pattern=pattern, **kwargs)
        snake.apple_location = pattern.apple
        # snake.reset()
        # snake = None
        snake.snake_array = deque(pattern.snake_body)
        snake._body_locations = set(pattern.snake_body)
        return snake
    ...
    def render(self, mode="human", drawing_vision=True, path=None):
        ...
        cv2.rectangle(self.img, (self.apple_location.x * size, self.apple_location.y * size),
                      (self.apple_location.x * size + size, self.apple_location.y * size + size), (255, 0, 0), -1,
